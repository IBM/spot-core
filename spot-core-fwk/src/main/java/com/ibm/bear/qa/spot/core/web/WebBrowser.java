...
/**
 * Print current browser cookies in debug output.
 *
 * @param title Title to put before printing the cookies list
 */
public void debugCookies(final String title) {
	debugPrintln("		  -> Current browser cookies "+title+":");
...
private void initWindow() {
	...
	final boolean isHeadless = this.manager.isHeadless();
	if (getParameterBooleanValue("windowMax", true) && !isHeadless) {
	...
	try {
		Dimension dim = new Dimension(
			getParameterIntValue("windowWidth", isHeadless ? MAX_WIDTH : DEFAULT_WIDTH),
			getParameterIntValue("windowHeight", isHeadless ? MAX_HEIGHT : DEFAULT_HEIGHT));
