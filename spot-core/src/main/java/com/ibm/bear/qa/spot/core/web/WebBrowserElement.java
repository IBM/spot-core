...
//	if (DEBUG) debugPrintln("			(move to "+this+", entirelyVisible="+entirelyVisible+")");
	...
	try {
		...
		while (true) {
			try {
				if (this.browser.manager.getType() == BROWSER_KIND_FIREFOX && !this.browser.getVersion().equals(FIREFOX_52ESR)) {
					// Unfortunately there's a bug in Firefox Gecko driver which was impacted by a Selenium change
					// See https://bugzilla.mozilla.org/show_bug.cgi?id=1489141 and https://github.com/SeleniumHQ/selenium/issues/6393
					// It seems that a workaround has been implemented which might be in next Gecko driver version,
					// but until then, there's no way to circumvent this issue
					// Tried to workaround my calling #scrollingIntoView() method but that seems
					// to screw up the browser, hence it has been commented...
//					scrollIntoView();
					// Hence the strategy becomes to do nothing when moving to an element with FF 60esr
					// When the move is used to make visible buttons, then use the #setVisibility(boolean) or setVisible(int) methods
					if (DEBUG) debugPrintln("			 ( -> NOTHING was done as Firefox 60esr is used and it does NOT support #moveToElement() methods.)");
				} else {
